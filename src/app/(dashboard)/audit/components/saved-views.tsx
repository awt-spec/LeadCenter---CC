...
export function SavedViews() {
  ...
  async function fetchViews() {
    setLoading(true);
    try {
      const res = await fetch('/api/audit/views');
  ...
  function applyView(v: View) {
    ...
    router.replace(`/audit${params.toString() ? '?' + params.toString() : ''}`, {
      scroll: false,
    });
    setOpen(false);
  }
  ...
  async function deleteView(v: View) {
    ...
    await fetch(`/api/audit/views/${v.id}`, { method: 'DELETE' });
    fetchViews();
  }
  ...
  async function saveCurrent() {
    ...
    setSaving(true);
    try {
      // Capturar filtros actuales del URL (incluye chips multi)
      const filters: Record<string, unknown> = {};
      const arrayKeys = ['userId', 'action', 'resource'];
      for (const k of arrayKeys) {
        const all = sp?.getAll(k) ?? [];
        if (all.length) filters[k] = all;
      }
      for (const k of ['dateFrom', 'dateTo', 'q']) {
        const v = sp?.get(k);
        if (v) filters[k] = v;
      }
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <Input
                      ...
                      placeholder="ej. Deletes últimos 7 días"
                    />
                  </div>
                ...
                >
                  <div className="flex-1 min-w-0">
                    <div className="text-sm text-sysde-gray flex items-center gap-1.5">
                      ...
                      {v.isShared ? (
                        <Globe2
                          className="h-3 w-3 text-sysde-mid"
