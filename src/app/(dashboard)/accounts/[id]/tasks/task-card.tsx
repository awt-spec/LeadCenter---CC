...
import { CalendarDays, MessageCircle, Paperclip, ListTree } from 'lucide-react';
...
const TAG_COLORS = ['bg-blue-50 text-blue-700', 'bg-emerald-50 text-emerald-700', 'bg-violet-50 text-violet-700', 'bg-amber-50 text-amber-700'];
...
export function TaskCard({
  ...
  return (
    <div
      ...
      onClick={onClick}
      className={cn(
        'cursor-grab rounded-lg border border-sysde-border bg-white p-3 text-left shadow-sm transition-all hover:border-sysde-red/40 hover:shadow active:cursor-grabbing',
        isDragging && 'shadow-lg'
      )}
    >
      <div className="flex items-start gap-2">
        <span
          className={cn('mt-1 h-2 w-2 shrink-0 rounded-full', TASK_PRIORITY_DOT[task.priority])}
          ...
          {task.dueDate && (
            <span
              className={cn(
                ...
                todayDue && 'font-semibold text-amber-600'
              )}
            >
